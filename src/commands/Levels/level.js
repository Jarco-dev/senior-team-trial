...
class XpCommand extends BaseCommand {
    constructor() {
        super({
            name: "level",
            description: "Lookup a users level",
            args: "[user]",
            examples: ["level", "level @user", "level 232163746829697025"],
            cooldown: 3000,
            disableDm: true,
            deleteMsg: true,
    ...
    async run(msg) {
        ...
        if (msg.args[0]) {
            ...
            try {
                ...
                if (userData) {
                    this.sender.send(msg, `**${user.username}** has a chat level of **${userData.level}**`);
                } else {
                    this.sender.invalid(msg, "This user hasn't received any xp yet");
                }
            } catch (err) {
                this.logger.error(err);
                this.sender.error(msg, "Something went wrong while getting this users level, please try again", 5000);
            }
        } else {
            try {
                const userData = await this.db.Users.findOne({ where: { userId: msg.author.id } });
                if (userData) {
                    this.sender.send(msg, `You have a chat level of **${userData.level}**`);
                } else {
